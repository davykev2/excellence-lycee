...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCSpaceGeometryPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.terminalCSpaceGeometryPath;
const strings = [];
...
for (const [location, value] of strings) {
  const formulas = value.matchAll(/\$\$([\s\S]+?)\$\$|\$([^$]+)\$/g);
...
console.log(
  `KaTeX valide : ${formulaCount} formules contrôlées dans ${path.modules[0].lessons.length} niveaux.`,
);
