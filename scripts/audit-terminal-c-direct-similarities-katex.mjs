...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCDirectSimilaritiesPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.terminalCDirectSimilaritiesPath;
const lessons = path.modules.flatMap((item) => item.lessons);
const expectedIds = [
  "similarity-definition",
  "similarity-composition-inverse",
  "similarity-invariants",
  "similarity-canonical",
  "similarity-complex-form",
  "similarity-center-form",
  "similarity-from-images",
  "similarity-center-construction",
  "similarity-applications",
];
const expectedWeights = [50, 55, 60, 65, 70, 75, 80, 85, 85];
const questionCount = lessons.reduce((total, lesson) => total + (lesson.questions?.length ?? 1), 0);

assert.deepEqual(
  lessons.map((lesson) => lesson.id),
  expectedIds,
  "Les identifiants historiques de la leçon 16 ont changé.",
);
assert.deepEqual(
  lessons.map((lesson) => lesson.xp),
  expectedWeights,
  "Les poids XP historiques de la leçon 16 ont changé.",
);
assert.equal(questionCount, 178, "Le nombre de questions publiées a changé sans mise à jour du descriptif.");
assert.ok(
...
assert.equal(
  ...
  4,
  "Les quatre schémas interactifs de la leçon doivent rester présents.",
);
...
const errors = [];
let formulaCount = 0;
const unrepairedFormulaFragments = ["kge0", "BA_nle10", "ane0", "kneq"];
  ...
  for (const match of value.matchAll(/\$\$([\s\S]+?)\$\$|\$([^$]+)\$/g)) {
    ...
    if (unrepairedFragment) {
      errors.push({
        location,
        tex,
        message: "Commande TeX non restaurée : " + unrepairedFragment,
      });
      continue;
    }
    if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(tex)) {
