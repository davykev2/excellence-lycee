...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCDFreeElectricalOscillationsPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.freeElectricalOscillationsPath;
...
const expectedWeights = [45, 55, 65, 75, 85, 90, 100, 115, 125];
const expectedQuestionCounts = [10, 11, 12, 14, 16, 10, 12, 15, 10];
const questionCount = lessons.reduce((total, lesson) => total + (lesson.questions?.length ?? 0), 0);

assert.equal(path.id, "terminale-cd-free-electrical-oscillations");
...
assert.equal(questionCount, 110, "La leçon doit publier 110 réponses évaluables.");
assert.ok(
...
assert.equal(
  lessons.find((lesson) => lesson.id === "natural-frequency-phase-quiz-one")?.interaction.formulaTex,
  "u_C(t)=5\\cos(100\\pi t)",
  "La formule interactive du Quiz 1 a perdu ses commandes LaTeX.",
);
assert.equal(
  lessons.find((lesson) => lesson.id === "negative-resistance-oscillation-maintenance")?.interaction.formulaTex,
  "R_{\\mathrm{eff}}=r-R_0",
  "La formule de résistance effective a perdu ses commandes LaTeX.",
);
assert.ok(
  lessons.every((lesson) => lesson.source?.documentTitle === "PHYS. Oscillations électriques libres dans un circuit LC.pdf"),
...
assert.ok(formulaCount >= 140, "La richesse scientifique de la leçon a diminué : " + formulaCount + " formules.");
