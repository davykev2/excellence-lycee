...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCComplexGeometryPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.terminalCComplexGeometryPath;
const lessons = path.modules.flatMap((item) => item.lessons);
const expectedIds = [
  "complex-angle",
  "complex-distance-ratio",
  "complex-loci",
  "complex-align-orthogonal",
  "complex-cyclic-triangles",
...
assert.deepEqual(lessons.map((lesson) => lesson.id), expectedIds, "Les identifiants historiques de la leçon 13 ont changé.");
assert.deepEqual(lessons.map((lesson) => lesson.xp), expectedWeights, "Les poids XP historiques de la leçon 13 ont changé.");
assert.equal(questionCount, 108, "Le nombre de questions publiées a changé sans mise à jour du descriptif.");
