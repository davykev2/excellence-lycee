...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCDAutoInductionPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.autoInductionPath;
...
const expectedQuestionCounts = [8, 10, 9, 9, 8, 12, 16, 18];
const questionCount = lessons.reduce((total, lesson) => total + (lesson.questions?.length ?? 0), 0);

assert.equal(path.id, "terminale-cd-auto-induction");
assert.deepEqual(path.levelIds, ["terminale-c", "terminale-d"]);
assert.equal(path.chapterNumber, 10);
assert.deepEqual(lessons.map((lesson) => lesson.id), expectedIds, "Les identifiants de la leçon 10 ont changé.");
...
assert.equal(questionCount, 90, "La leçon doit publier 90 réponses évaluables.");
assert.ok(
  lessons.every((lesson) => (lesson.concept.bodyMarkdown?.length ?? 0) >= 1_000),
  "Chaque niveau doit conserver un cours substantiel.",
);
assert.equal(lessons.filter((lesson) => lesson.interaction.kind === "curve").length, 4);
...
assert.equal(lessons.filter((lesson) => lesson.source?.fidelity === "faithful-corrected").length, 6);
assert.equal(
  lessons.find((lesson) => lesson.id === "self-induced-emf-current-slopes")?.interaction.formulaTex,
  "e(t)=-L\\,\\frac{\\mathrm di}{\\mathrm dt}",
  "La formule interactive de la f.é.m. a perdu ses commandes LaTeX.",
);
assert.equal(
  lessons.find((lesson) => lesson.id === "evaluation-piecewise-current-voltage")?.interaction.formulaTex,
  "u_{AB}=ri+L\\,\\frac{\\mathrm di}{\\mathrm dt}",
  "La formule interactive de la tension a perdu ses commandes LaTeX.",
);
assert.equal(
  lessons.find((lesson) => lesson.id === "solenoid-synthesis-spark-mission")?.interaction.formulaTex,
  "u_{MN}=L\\,\\frac{\\mathrm di}{\\mathrm dt}",
  "La formule interactive de la mission a perdu ses commandes LaTeX.",
);
assert.ok(
  lessons.every((lesson) => lesson.source?.documentTitle === "TleD_PHY_L10_Auto induction.pdf"),
