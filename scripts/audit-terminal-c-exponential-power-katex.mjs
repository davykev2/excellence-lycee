...
const source = readFileSync(
  new URL("../apps/web/src/data/terminalCExponentialPowerPath.ts", import.meta.url),
  "utf8",
);
...
const path = module.exports.terminalCExponentialPowerPath;
...
assert.deepEqual(lessons.map((lesson) => lesson.id), expectedIds, "Les identifiants historiques de la leçon 10 ont changé.");
assert.deepEqual(lessons.map((lesson) => lesson.xp), expectedWeights, "Les poids XP historiques de la leçon 10 ont changé.");

const questionCount = lessons.reduce((total, lesson) => total + (lesson.questions?.length ?? 1), 0);
assert.equal(questionCount, 128, "Le nombre de questions publiées a changé sans mise à jour du descriptif.");
const strings = [];
