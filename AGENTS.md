...
1. Appliquer le même traitement d'enrichissement aux leçons restantes des autres séries. **La Terminale A (maths) est terminée : ses 8 leçons sont toutes enrichies. La série Histoire (H2 à H9) est terminée** (partagée A/C/D via la fabrique Humanités). En Terminale C, les leçons de maths 01 à 17 sont enrichies ; seules les leçons 18 et 19 utilisent encore le générateur compact. Plusieurs leçons de Terminale D bénéficient déjà des parcours C partagés, mais son catalogue n'est pas encore entièrement enrichi depuis ses propres PDF.
