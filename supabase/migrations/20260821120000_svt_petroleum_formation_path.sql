-- Publie la leçon de SVT « La mise en place des gisements pétrolifères
-- en Côte d'Ivoire » (Terminale C, thème Les ressources énergétiques).
--
...
-- (apps/web/src/data/terminalCSvtPetroleumFormationPath.ts) et du registre XP
-- de l'API (apps/api/src/curriculum.ts).
-- Répartition normalisée attendue :
--   620 / 750 / 820 / 960 / 1030 / 1160 / 1300 / 1510 / 1850 = 10 000.

create temporary table svt_petroleum_formation_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_petroleum_formation_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l9-petroleum-formation', '["ivorian-sedimentary-basin-location","source-reservoir-seal-rocks","organic-deposition-kerogen","thermal-maturation-hydrocarbons","primary-secondary-migration","fluid-sorting-reservoir-trap","stratigraphic-structural-mixed-traps","official-gap-order-exercises","anticline-wells-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_petroleum_formation_manifest as manifest
...
with reward_weights as (
  ...
  join svt_petroleum_formation_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
