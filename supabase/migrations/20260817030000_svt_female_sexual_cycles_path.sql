-- Publie la leçon de SVT « Les cycles sexuels chez la femme »
-- (Terminale C, thème La reproduction chez l'être humain).
--
...
-- (apps/web/src/data/terminalCSvtFemaleCyclesPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   670 / 810 / 890 / 960 / 1040 / 1110 / 1260 / 1480 / 1780 = 10 000.

create temporary table svt_female_cycles_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_female_cycles_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l7-female-sexual-cycles', '["cycle-landmarks-menarche","ovarian-cycle-follicle-ovulation","uterine-cycle-endometrium","pituitary-ovarian-hormone-curves","experimental-control-ovary-pituitary","hypothalamic-pituitary-ovarian-axis","negative-positive-feedback","rabbit-endometrium-assessment","integrated-cycle-final-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 60, 65, 70, 75, 85, 100, 120])[expanded.position]::integer as xp_awarded
  from svt_female_cycles_manifest as manifest
...
with reward_weights as (
  ...
  join svt_female_cycles_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
