-- Publie la leçon de SVT « L'utilisation de l'énergie par la cellule musculaire »
-- (Terminale C, thème Le métabolisme énergétique et l'activité musculaire).
--
...
-- (apps/web/src/data/terminalCSvtMuscleEnergyPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   610 / 680 / 750 / 880 / 1020 / 1090 / 1160 / 950 / 1290 / 1570 = 10 000.

create temporary table svt_muscle_energy_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_muscle_energy_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l4-muscle-energy-use', '["muscle-fiber-organization","sarcomere-bands-filaments","sarcomere-shortening-evidence","excitation-calcium-coupling","actomyosin-cross-bridge-cycle","immediate-atp-regeneration","glycolysis-lactate-respiration","muscle-heat-production","trained-untrained-lactate-assessment","muscle-response-fatigue-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 50, 55, 65, 75, 80, 85, 70, 95, 115])[expanded.position]::integer as xp_awarded
  from svt_muscle_energy_manifest as manifest
...
with reward_weights as (
  ...
  join svt_muscle_energy_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
