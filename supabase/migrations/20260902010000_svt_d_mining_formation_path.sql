-- Publie la leçon de SVT « La mise en place des gisements miniers en Côte d’Ivoire »
-- (Terminale D, chapitre 13 du catalogue).
--
-- C’est une création : le parcours n’existait pas encore. Cette migration
-- n’effectue donc aucune suppression et ne touche aucun autre parcours.
--
-- Les poids bruts ci-dessous sont identiques à ceux du frontend
-- (apps/web/src/data/terminalDSvtMiningFormationPath.ts) et du registre XP
-- de l’API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   620 / 750 / 820 / 960 / 1030 / 1160 / 1300 / 1510 / 1850 = 10 000.

create temporary table svt_d_mining_formation_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_mining_formation_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l13-mining-formation', '["ivorian-mineral-resources-map","host-rock-mineral-ore-vocabulary","primary-gold-deposit-formation","weathering-gold-liberation","transport-density-sorting","secondary-alluvial-gold-deposit","primary-secondary-deposit-comparison","gold-deposit-synthesis-annotation","mineral-deposit-final-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 60, 70, 75, 85, 95, 110, 135])[expanded.position]::integer as xp_awarded
  from svt_d_mining_formation_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_mining_formation_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l13-mining-formation';
