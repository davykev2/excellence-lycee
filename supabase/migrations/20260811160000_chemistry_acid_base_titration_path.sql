-- Publie la leçon de chimie 11 « Dosage acido-basique »
...
-- (apps/web/src/data/terminalCDAcidBaseTitrationPath.ts) et du registre XP de l'API
...
create temporary table acid_base_titration_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into acid_base_titration_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-acid-base-titration', '["titration-principle","colorimetric-vinegar","concentration-from-equivalence","indicator-choice","species-at-equivalence","ph-metric-titration","titration-mission"]');
...
with raw_rewards as (
  ...
  from acid_base_titration_manifest as manifest
...
with reward_weights as (
  ...
  join acid_base_titration_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
