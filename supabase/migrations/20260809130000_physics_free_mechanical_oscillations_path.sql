-- Publie la leçon 05 de Physique « Oscillations mécaniques libres »
...
--   770 / 950 / 1120 / 1210 / 1210 / 1380 / 1550 / 1810 = 10 000 XP.

create temporary table free_mechanical_oscillations_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into free_mechanical_oscillations_manifest (path_id, lesson_ids) values
  ('terminale-cd-free-mechanical-oscillations', '["free-oscillation-basics","spring-mass-force-model","free-oscillation-equation","harmonic-solution-initial-conditions","oscillation-graphs-phase","mechanical-energy-conservation","official-fixation-exercises","suspension-oscillator-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 70, 70, 80, 90, 105])[expanded.position]::integer as xp_awarded
  from free_mechanical_oscillations_manifest as manifest
...
with reward_weights as (
  ...
  join free_mechanical_oscillations_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
