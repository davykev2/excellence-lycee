-- Publie la leçon 08 de Physique « Loi de Laplace »
...
--   760 / 930 / 1020 / 1100 / 1270 / 1360 / 1610 / 1950 = 10 000 XP.

create temporary table laplace_law_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into laplace_law_manifest (path_id, lesson_ids) values
  ('terminale-cd-laplace-law', '["laplace-force-experiments","laplace-vector-law-characteristics","laplace-force-direction","laplace-force-magnitude","cotton-balance-measurement","laplace-force-applications","official-laplace-exercises","laplace-measurement-equilibrium-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 60, 65, 75, 80, 95, 115])[expanded.position]::integer as xp_awarded
  from laplace_law_manifest as manifest
...
with reward_weights as (
  ...
  join laplace_law_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
