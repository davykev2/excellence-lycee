-- Publie la leçon de chimie 07 « Acide fort - base forte »
...
-- (apps/web/src/data/terminalCDStrongAcidBasePath.ts) et du registre XP de l'API
...
create temporary table strong_acid_base_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into strong_acid_base_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-strong-acid-base', '["strong-acid-definition","strong-acid-ph","strong-base-definition","strong-base-ph","species-inventory-electroneutrality","mixtures-and-dilution","acid-mixture-mission"]');
...
with raw_rewards as (
  ...
  from strong_acid_base_manifest as manifest
...
with reward_weights as (
  ...
  join strong_acid_base_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
