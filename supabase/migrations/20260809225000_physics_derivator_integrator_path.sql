-- Publie la leçon de Physique « Montages dérivateur et intégrateur »
-- (leçon 11 en Terminale C et leçon 9 en Terminale D, thème Électricité).
--
...
create temporary table derivator_integrator_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into derivator_integrator_manifest (path_id, lesson_ids) values
  ('terminale-cd-derivator-integrator', '["ideal-operational-amplifier","derivator-circuit-identification","derivator-law-slope-method","derivator-quiz-triangular-signal","integrator-circuit-law","integrator-quiz-square-signal","derivator-integrator-comparison","integrator-evaluation-signal-reconstruction"]');
...
with raw_rewards as (
  ...
  from derivator_integrator_manifest as manifest
...
with reward_weights as (
  ...
  join derivator_integrator_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
