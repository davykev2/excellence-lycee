-- Publie la leçon de SVT « Le fonctionnement du cœur »
...
-- (apps/web/src/data/terminalDSvtHeartPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP
-- attribué. Répartition normalisée attendue :
--   550 / 670 / 800 / 860 / 920 / 1040 / 1100 / 1170 / 1290 / 1600 = 10 000.

create temporary table svt_d_heart_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_heart_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l4-heart', '["heart-isolated-automaticity","heart-conduction-system","pacemaker-myocyte-potentials","cardiac-cycle-mechanics","ecg-electromechanical-coupling","autonomic-efferent-control","baroreflex-sino-aortic","loewi-chemical-mediation","cardiac-mediators-pharmacology","heart-function-final-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 70, 75, 85, 90, 95, 105, 130])[expanded.position]::integer as xp_awarded
  from svt_d_heart_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_heart_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
