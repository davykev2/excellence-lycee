-- Publie la leçon de SVT « Le réflexe conditionnel »
-- (Terminale D, thème La communication dans l'organisme).
--
...
-- (apps/web/src/data/terminalDSvtConditionedReflexPath.ts) et du registre XP
-- de l'API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   550 / 670 / 790 / 850 / 910 / 980 / 1100 / 1220 / 1340 / 1590 = 10 000.

create temporary table svt_d_conditioned_reflex_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_conditioned_reflex_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l1-conditioned-reflex', '["reflex-response-foundations","pavlov-conditioning-experiment","conditioned-reflex-acquisition","temporal-association-repetition","reinforcement-conditioning-maintenance","extinction-spontaneous-recovery","generalization-discrimination","cortical-plasticity-neural-pathways","conditioned-reflex-applications-limits","conditioned-reflex-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_conditioned_reflex_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_conditioned_reflex_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
