-- Publie la leçon 03 de Physique « Interaction gravitationnelle »
-- (Terminale C uniquement, thème Mécanique).
--
...
-- (apps/web/src/data/terminalCGravitationPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   1100 / 1340 / 1580 / 1710 / 1950 / 2320 = 10 000.
-- Les six poids (45, 55, 65, 70, 80, 95) étant distincts, aucune partie
-- fractionnaire n'est partagée : la règle de départage `lesson_id asc` n'a aucun effet.

create temporary table gravitation_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into gravitation_manifest (path_id, lesson_ids) values
  ('terminale-c-gravitation', '["newton-law","gravitational-field","satellite-motion","geostationary-satellite","kepler-laws","earth-mass-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 70, 80, 95])[expanded.position]::integer as xp_awarded
  from gravitation_manifest as manifest
...
with reward_weights as (
  ...
  join gravitation_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
