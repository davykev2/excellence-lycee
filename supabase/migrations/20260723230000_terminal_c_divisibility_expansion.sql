-- Étend la leçon 03 de Terminale C « Divisibilité dans Z » de 8 à 12 niveaux.
-- Les huit identifiants historiques sont conservés. Quatre blocs distincts sont
-- ajoutés : critères de divisibilité, stratégie modulaire, codage affine et
-- codage exponentiel.
--
-- Le parcours distribue toujours exactement 10 000 XP. Les progressions
-- existantes sont réalignées sans perdre le meilleur score des élèves.

create temporary table terminal_c_divisibility_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into terminal_c_divisibility_manifest (path_id, lesson_ids) values
  ('terminale-c-math-l03-divisibility', '["integer-divisibility","euclidean-division-z","remainders-operations","congruences","numeration-divisibility-tests","divisibility-tests","prime-numbers","prime-factorization","number-of-divisors","modular-strategy-mission","affine-coding-mission","exponential-coding-mission"]');
...
with raw_rewards as (
  ...
  from terminal_c_divisibility_manifest as manifest
...
with reward_weights as (
  ...
  join terminal_c_divisibility_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
