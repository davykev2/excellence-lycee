-- Publie la leçon de SVT « Le maintien de la constance du milieu intérieur »
-- (Terminale D, chapitre global 10 de la progression officielle 2025-2026).
--
...
-- (apps/web/src/data/terminalDSvtInternalEnvironmentPath.ts) et du registre XP
-- de l'API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   520 / 640 / 760 / 870 / 990 / 1050 / 1100 / 1160 / 1280 / 1630 = 10 000.

create temporary table svt_d_internal_environment_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_internal_environment_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l10-internal-environment', '["renal-homeostasis-architecture","nephron-structure-urine-pathway","glomerular-filtration-selectivity","tubular-reabsorption-secretion-excretion","water-load-diuresis-osmolarity","adh-water-balance-feedback","renin-angiotensin-aldosterone-system","renal-acid-base-regulation","renal-experiments-critical-analysis","internal-environment-final-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 75, 85, 90, 95, 100, 110, 140])[expanded.position]::integer as xp_awarded
  from svt_d_internal_environment_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_internal_environment_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
