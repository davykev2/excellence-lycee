-- Publie la leçon de Physique « Puissance en courant alternatif »
-- (leçon 15 en Terminale C et leçon 13 en Terminale D, thème Électricité).
--
...
create temporary table ac_power_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into ac_power_manifest (path_id, lesson_ids) values
  ('terminale-cd-ac-power', '["ac-power-instantaneous","ac-power-average-apparent-energy","ac-power-factor-transport","ac-power-rlc-frequency","ac-power-half-power-band","ac-power-official-exercises-one-two","ac-power-official-exercise-three","ac-power-official-exercise-four","ac-power-official-exercise-five-grid-mission"]');
...
with raw_rewards as (
  ...
  from ac_power_manifest as manifest
...
with reward_weights as (
  ...
  join ac_power_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
