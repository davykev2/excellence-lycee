-- Synchronise le parcours de Chimie « Fabrication d'un savon »
-- (Terminales C et D) publié dans le commit 1bf58e5.
--
-- La migration ne modifie aucun contenu pédagogique et ne supprime aucune
-- progression. Elle crée uniquement son barème XP serveur manquant.
-- Répartition normalisée attendue :
--   950 / 1160 / 1260 / 1370 / 1580 / 1680 / 2000 = 10 000 XP.

create temporary table chemistry_soap_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into chemistry_soap_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-soap', '["saponification-definition","triglycerides-structure","soap-equation","soap-preparation-protocol","saponification-calculations","saponification-workshop","soap-production-mission"]');
...
with raw_rewards as (
  ...
  from chemistry_soap_manifest as manifest
...
with reward_weights as (
  ...
  join chemistry_soap_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
