-- Publie la leçon de SVT « L’amélioration et la protection des sols »
-- (Terminale D, chapitre 15 du catalogue ; le fichier source porte « L15 »
-- et sa couverture « Leçon 3 »).
--
...
-- (apps/web/src/data/terminalDSvtSoilProtectionPath.ts) et du registre XP
-- de l’API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   550 / 670 / 790 / 850 / 910 / 980 / 1100 / 1220 / 1340 / 1590 = 10 000.

create temporary table svt_d_soil_protection_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_soil_protection_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l15-soil-protection', '["soil-fertility-diagnosis","mineral-fertilizer-dose-response","earthworms-organic-mineralization","green-manure-nitrogen-cycle","acid-soil-degradation-diagnosis","lime-exchange-neutralization","humus-clay-soil-properties","soil-protection-practices","soil-official-application-exercises","soil-management-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_soil_protection_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_soil_protection_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l15-soil-protection';
