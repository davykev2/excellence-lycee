-- Publie la leçon de chimie 08 « Acide faible - base faible »
...
-- (apps/web/src/data/terminalCDWeakAcidBasePath.ts) et du registre XP de l'API
...
create temporary table weak_acid_base_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into weak_acid_base_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-weak-acid-base', '["weak-acid-evidence","weak-acid-quantitative","ionization-coefficient","weak-base-study","chemical-equilibrium-water-role","weak-acid-base-workshop","dilution-mission"]');
...
with raw_rewards as (
  ...
  from weak_acid_base_manifest as manifest
...
with reward_weights as (
  ...
  join weak_acid_base_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
