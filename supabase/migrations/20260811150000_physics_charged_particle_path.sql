...
-- (leçon 07 en Terminale C, leçon 06 en Terminale D).
--
...
create temporary table charged_particle_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into charged_particle_manifest (path_id, lesson_ids) values
  ('terminale-cd-charged-particle-magnetic-field', '["charged-particle-lorentz-force","charged-particle-lorentz-orientation","charged-particle-circular-motion","charged-particle-magnetic-deflection","charged-particle-mass-spectrograph","charged-particle-cyclotron-principle","charged-particle-cyclotron-energy","charged-particle-wien-filter","charged-particle-instruments-mission"]');
...
with raw_rewards as (
  ...
  from charged_particle_manifest as manifest
...
with reward_weights as (
  ...
  join charged_particle_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
