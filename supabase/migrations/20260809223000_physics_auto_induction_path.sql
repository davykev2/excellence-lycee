-- Publie la leçon 10 de Physique « Auto-induction »
...
create temporary table auto_induction_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into auto_induction_manifest (path_id, lesson_ids) values
  ('terminale-cd-auto-induction', '["auto-induction-evidence-transients","self-flux-solenoid-inductance","self-induced-emf-current-slopes","coil-terminal-voltage","magnetic-energy-power","evaluation-piecewise-current-voltage","official-auto-induction-exercises","solenoid-synthesis-spark-mission"]');
...
with raw_rewards as (
  ...
  from auto_induction_manifest as manifest
...
with reward_weights as (
  ...
  join auto_induction_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
