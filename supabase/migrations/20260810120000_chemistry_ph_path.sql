-- Publie la leçon de chimie 06 « Solutions aqueuses : notion de pH »
...
-- (apps/web/src/data/terminalCDAqueousSolutionsPhPath.ts) et du registre XP de l'API
...
create temporary table ph_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into ph_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-ph', '["water-solvent-properties","water-autoprotolysis","dissolution-process","solution-concentrations","ph-definition-classification","ph-concentration-workshop","ionic-mixture-mission"]');
...
with reward_weights as (
  ...
  join ph_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
