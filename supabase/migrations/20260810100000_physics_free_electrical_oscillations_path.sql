-- Publie la leçon de Physique « Oscillations électriques libres dans un circuit LC »
-- (leçon 12 en Terminale C et leçon 10 en Terminale D, thème Électricité).
--
...
--   600 / 730 / 860 / 990 / 1130 / 1190 / 1320 / 1520 / 1660 = 10 000 XP.

create temporary table free_electrical_oscillations_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into free_electrical_oscillations_manifest (path_id, lesson_ids) values
  ('terminale-cd-free-electrical-oscillations', '["electrical-oscillator-capacitor-charge","capacitor-discharge-damping-regimes","ideal-lc-differential-equation","natural-frequency-phase-quiz-one","lc-energy-conservation-quiz-two","mechanical-electrical-analogy","negative-resistance-oscillation-maintenance","lc-official-evaluation-mission","lc-consolidation-data-audit"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 75, 85, 90, 100, 115, 125])[expanded.position]::integer as xp_awarded
  from free_electrical_oscillations_manifest as manifest
...
with reward_weights as (
  ...
  join free_electrical_oscillations_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
