-- Enrichit la leçon 07 « Coniques » de Terminale C.
-- Les six identifiants historiques sont conservés dans le même ordre ;
-- cinq niveaux ajoutent la piscine elliptique et tous les exercices officiels
-- de fixation, de renforcement et d’approfondissement.

create temporary table terminal_c_conics_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into terminal_c_conics_manifest (path_id, lesson_ids) values
  (
    'terminale-c-math-l07-conics',
    '["conic-focus-directrix","conic-axis-vertices","conic-region","parabola-reduced-equation","ellipse-reduced-equation","hyperbola-reduced-equation","pool-ellipse-mission","official-fixation-workshop","parabola-construction-reinforcement","conic-from-focus-directrix-reinforcement","complex-hyperbola-mission"]'
  );
...
with raw_rewards as (
  ...
  from terminal_c_conics_manifest as manifest
...
using terminal_c_conics_manifest as manifest
...
with reward_weights as (
  ...
  join terminal_c_conics_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
from public.lesson_rewards as reward
join terminal_c_conics_manifest as manifest
