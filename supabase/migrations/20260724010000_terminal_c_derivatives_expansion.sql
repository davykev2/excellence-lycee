-- Enrichit la leçon « Dérivabilité et étude de fonctions » de Terminale C.
-- La leçon 03 de Terminale D réutilise le même parcours : les deux registres
-- de récompenses sont donc recalculés ensemble.

create temporary table terminal_derivatives_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into terminal_derivatives_manifest (path_id, lesson_ids) values
  (
    'terminale-c-math-l04-derivatives-functions',
    '["one-sided-derivatives","derivative-at-junction","vertical-half-tangent","derivative-on-interval-and-rules","derivative-composition","inverse-function-derivative","successive-derivatives","finite-increments","lipschitz-bound","complete-piecewise-function-study","radical-function-study","periodic-tangent-study","cocoa-profit-mission","corrected-exercises-workshop","reinforcement-exercises-mission"]'
  ),
  (
    'terminale-d-math-l03-derivatives-functions',
    '["one-sided-derivatives","derivative-at-junction","vertical-half-tangent","derivative-on-interval-and-rules","derivative-composition","inverse-function-derivative","successive-derivatives","finite-increments","lipschitz-bound","complete-piecewise-function-study","radical-function-study","periodic-tangent-study","cocoa-profit-mission","corrected-exercises-workshop","reinforcement-exercises-mission"]'
  );
...
with raw_rewards as (
  ...
  from terminal_derivatives_manifest as manifest
...
-- Supprime un éventuel niveau obsolète qui ne figurerait plus dans le manifeste.
delete from public.lesson_rewards as reward
using terminal_derivatives_manifest as manifest
...
with reward_weights as (
  ...
  join terminal_derivatives_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
