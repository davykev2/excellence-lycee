-- Publie la leçon de chimie 10 « Réactions acido-basiques et solutions tampons »
-- (Terminale C et D, thème Chimie générale).
--
-- C'est une création : le parcours n'existait pas encore. Cette migration n'effectue
-- donc aucune suppression et ne touche aucune progression existante. Elle se contente
-- d'insérer les récompenses des sept niveaux, puis de les normaliser à 10 000 XP.
--
-- Les poids bruts ci-dessous sont identiques à ceux du frontend
-- (apps/web/src/data/terminalCDAcidBaseBuffersPath.ts) et du registre XP de l'API
...
-- Les quatre bonus de répartition tombent sur les poids 75, 65, 55 et 45, dont les
-- parties fractionnaires sont toutes distinctes : la règle de départage par
-- `lesson_id` n'a donc aucun effet ici.

create temporary table acid_base_buffers_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into acid_base_buffers_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-acid-base-buffers', '["strong-acid-strong-base","strong-acid-titration-curve","weak-acid-strong-base","weak-acid-titration-curve","strong-acid-weak-base","buffer-solutions","titration-identification-mission"]');
...
with raw_rewards as (
  ...
  from acid_base_buffers_manifest as manifest
...
with reward_weights as (
  ...
  join acid_base_buffers_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
