-- Publie la leçon de SVT « L'amélioration de la fertilité du sol »
-- (Terminale C, thème La gestion des sols).
--
...
-- (apps/web/src/data/terminalCSvtSoilFertilityPath.ts) et du registre XP
-- de l'API (apps/api/src/curriculum.ts).
-- Répartition normalisée attendue :
--   570 / 700 / 830 / 960 / 1080 / 1210 / 1340 / 1460 / 1850 = 10 000.

create temporary table svt_soil_fertility_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_soil_fertility_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l11-soil-fertility', '["soil-fertility-diagnosis","water-air-regulation","cultural-techniques","fertilizer-dose-balance","organic-amendments","calcareous-amendment","humiferous-amendment","fertilizer-benefits-risks","cooperative-garden-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 75, 85, 95, 105, 115, 145])[expanded.position]::integer as xp_awarded
  from svt_soil_fertility_manifest as manifest
...
with reward_weights as (
  ...
  join svt_soil_fertility_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
