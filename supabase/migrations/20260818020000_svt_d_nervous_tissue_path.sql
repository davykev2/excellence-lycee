-- Publie la leçon de SVT « Le fonctionnement du tissu nerveux »
...
-- (apps/web/src/data/terminalDSvtNervousTissuePath.ts) et du registre XP de l'API
...
with raw_rewards as (
  ...
  from svt_d_nervous_tissue_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_nervous_tissue_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
