-- Publie la leçon de SVT « La transmission de deux caractères héréditaires
-- chez les êtres vivants » (Terminale D, chapitre 9 du catalogue actuel).
-- Le support complet porte encore le numéro historique L12 ; les identifiants
-- du parcours suivent le catalogue produit sans modifier ce document source.
--
...
-- (apps/web/src/data/terminalDSvtTwoTraitHeredityPath.ts). Le registre XP de
-- l'API est synchronisé lors de la consolidation centrale du lot coordonné.
-- Répartition normalisée attendue :
--   480 / 590 / 690 / 740 / 800 / 850 / 960 / 1060 / 1170 / 1280 / 1380 = 10 000.

create temporary table svt_d_two_trait_heredity_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_two_trait_heredity_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l9-two-trait-heredity', '["dihybridism-evidence","pea-pure-lines-f1","independent-gametes","pea-f2-nine-three-three-one","pea-test-cross","independence-check-method","drosophila-linked-test-cross","cis-crossing-over","genetic-distance-map","genetic-applications-supplement","two-trait-heredity-final-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 70, 75, 80, 90, 100, 110, 120, 130])[expanded.position]::integer as xp_awarded
  from svt_d_two_trait_heredity_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_two_trait_heredity_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
-- barème normalisé si cette migration est rejouée après une activité locale.
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l9-two-trait-heredity';
