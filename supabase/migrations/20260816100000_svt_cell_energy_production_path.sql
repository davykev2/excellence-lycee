-- Publie la leçon de SVT « La production d'énergie par la cellule »
...
-- (apps/web/src/data/terminalCSvtCellEnergyPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   680 / 830 / 900 / 980 / 1050 / 1200 / 1280 / 1430 / 1650 = 10 000.

create temporary table svt_cell_energy_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_cell_energy_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l3-cell-energy-production', '["aerobic-respiration-experiment","anaerobic-fermentation-experiment","respiration-fermentation-comparison","mitochondrion-evidence-structure","glycolysis-stage","pyruvate-krebs-cycle","respiratory-chain-atp-balance","isolated-mitochondria-assessment","muscle-energy-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 60, 65, 70, 80, 85, 95, 110])[expanded.position]::integer as xp_awarded
  from svt_cell_energy_manifest as manifest
...
with reward_weights as (
  ...
  join svt_cell_energy_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
