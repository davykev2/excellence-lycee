-- Publie la leçon de SVT « La communication nerveuse » (Terminale C, thème La
...
-- (apps/web/src/data/terminalCSvtNervousPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   840 / 1030 / 1310 / 1310 / 1120 / 1210 / 1400 / 1780 = 10 000.
--
-- Deux niveaux partagent le même poids (70) et donc la même partie fractionnaire.
-- Cette égalité est sans conséquence : tous deux figurent parmi les quatre unités
-- de bonus attribuées, et la frontière du bonus (0,5701 contre 0,4953) est nette.
-- La règle de départage `lesson_id asc` n'a donc aucun effet sur le résultat.

create temporary table svt_nervous_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_nervous_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l1-nervous-communication', '["neuron-structure","resting-potential","action-potential-phases","ionic-explanation","local-currents","synapse-types","synaptic-transmission","neuron-chain-mission"]');

-- 1. Écrit les poids bruts de progression des huit niveaux.
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 70, 70, 60, 65, 75, 95])[expanded.position]::integer as xp_awarded
  from svt_nervous_manifest as manifest
...
with reward_weights as (
  ...
  join svt_nervous_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
