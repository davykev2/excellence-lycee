-- Publie la leçon de SVT « Le fonctionnement des organes sexuels chez l'Homme »
-- (Terminale D, chapitre 6 du catalogue). Sa portée est explicitement mixte :
-- cycles sexuels féminins, fonctionnement testiculaire et régulations communes.
--
...
-- (apps/web/src/data/terminalDSvtHumanSexualOrgansPath.ts) et du registre XP
-- de l'API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   550 / 670 / 790 / 850 / 910 / 980 / 1100 / 1220 / 1340 / 1590 = 10 000.

create temporary table svt_d_human_sexual_organs_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_human_sexual_organs_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l6-human-sexual-organs', '["female-sexual-cycle-landmarks","ovarian-follicular-ovulatory-luteal-cycle","uterine-menstrual-proliferative-secretory-cycle","ovarian-pituitary-hormone-curves","hypophysectomy-ovariectomy-experiments","hypothalamic-pituitary-ovarian-control","female-negative-positive-feedback","male-hypothalamic-pituitary-testicular-control","hormonal-contraception-mechanisms","human-sexual-organs-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_human_sexual_organs_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_human_sexual_organs_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l6-human-sexual-organs';
