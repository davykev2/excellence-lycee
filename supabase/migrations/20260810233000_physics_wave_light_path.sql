-- Publie la leçon 16 de Physique en Terminale C :
...
create temporary table wave_light_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into wave_light_manifest (path_id, lesson_ids) values
  ('terminale-c-wave-light', '["wave-light-diffraction-observation","wave-light-diffraction-geometry","wave-light-young-interference","wave-light-path-difference-fringes","wave-light-electromagnetic-spectrum","wave-light-official-evaluation-laser","wave-light-official-young-diffraction","wave-light-frequency-table-lab","wave-light-measurement-spectrum-mission"]');
...
with raw_rewards as (
  ...
  from wave_light_manifest as manifest
...
with reward_weights as (
  ...
  join wave_light_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
