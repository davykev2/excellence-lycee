-- Publie la leçon de chimie 02 « Composés carbonylés : aldéhydes et cétones »
-- (Terminale C et D, thème Chimie organique).
--
...
-- (apps/web/src/data/terminalCDCarbonylCompoundsPath.ts) et du registre XP de l'API
...
create temporary table carbonyl_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into carbonyl_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-carbonyl', '["carbonyl-definition","carbonyl-nomenclature","dnph-common-test","aldehyde-differentiating-tests","carbonyl-redox-equations","carbonyl-identification-workshop","unknown-carbonyl-mission"]');
...
with raw_rewards as (
  ...
  from carbonyl_manifest as manifest
...
with reward_weights as (
  ...
  join carbonyl_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
