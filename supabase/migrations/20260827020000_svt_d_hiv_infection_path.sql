...
-- (Terminale D, chapitre 12 du catalogue ; le fichier source porte « L7 » et sa couverture « Leçon 15 »).
--
...
-- (apps/web/src/data/terminalDSvtHivInfectionPath.ts) et du registre XP
-- de l’API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   640 / 780 / 920 / 990 / 1060 / 1210 / 1280 / 1420 / 1700 = 10 000.

create temporary table svt_d_hiv_infection_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_hiv_infection_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l12-hiv-infection', '["hiv-virion-architecture","hiv-cd4-coreceptor-entry","hiv-reverse-transcription-integration","hiv-expression-assembly-maturation","hiv-acute-infection-seroconversion","hiv-chronic-phase-immune-depletion","hiv-transmission-prevention-treatment","hiv-official-cycle-exercises","hiv-diagnosis-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_hiv_infection_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_hiv_infection_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l12-hiv-infection';
