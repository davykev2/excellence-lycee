-- Publie la leçon de SVT « Le fonctionnement du muscle strié squelettique »
...
-- (apps/web/src/data/terminalDSvtSkeletalMusclePath.ts) et du registre XP de
-- l'API (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP
-- attribué. Répartition normalisée attendue :
--   550 / 670 / 790 / 850 / 910 / 980 / 1100 / 1220 / 1340 / 1590 = 10 000.

create temporary table svt_d_skeletal_muscle_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_skeletal_muscle_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l3-skeletal-muscle', '["muscle-organ-to-fiber","sarcomere-bands-myofilaments","sarcomere-sliding-contraction","excitation-calcium-coupling","actomyosin-cross-bridge-cycle","action-potential-muscle-twitch","summation-tetanus-fatigue","rapid-atp-regeneration","glycolysis-respiration-energy","official-muscle-assessment-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_skeletal_muscle_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_skeletal_muscle_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
