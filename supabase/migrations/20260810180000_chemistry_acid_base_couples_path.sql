-- Publie la leçon de chimie 09 « Couples acide-base : classification »
...
-- (apps/web/src/data/terminalCDAcidBaseCouplesPath.ts) et du registre XP de l'API
...
create temporary table acid_base_couples_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into acid_base_couples_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-acid-base-couples', '["bronsted-couples","acidity-constant-pka","pka-determination","predominance-indicators","acid-base-strength","couples-classification","pka-determination-mission"]');
...
with raw_rewards as (
  ...
  from acid_base_couples_manifest as manifest
...
with reward_weights as (
  ...
  join acid_base_couples_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
