-- Publie la leçon 06 de Physique « Champ magnétique »
-- (Terminales C et D, thème Électricité).
--
...
--   780 / 870 / 1050 / 1130 / 1220 / 1390 / 1650 / 1910 = 10 000 XP.

create temporary table magnetic_field_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into magnetic_field_manifest (path_id, lesson_ids) values
  ('terminale-cd-magnetic-field', '["magnetic-interactions-sources","magnetic-vector-characteristics","magnetic-field-lines-spectra","solenoid-orientation","solenoid-field-law","magnetic-field-superposition","official-solenoid-exercises","solenoid-measurement-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 50, 60, 65, 70, 80, 95, 110])[expanded.position]::integer as xp_awarded
  from magnetic_field_manifest as manifest
...
with reward_weights as (
  ...
  join magnetic_field_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
