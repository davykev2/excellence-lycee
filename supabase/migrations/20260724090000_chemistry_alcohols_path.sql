-- Publie la leçon de chimie « Les alcools » (Terminale C et D, thème Chimie organique).
--
...
-- (apps/web/src/data/chemistryPaths.ts) et du registre XP de l'API
...
create temporary table alcohols_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into alcohols_manifest (path_id, lesson_ids) values
  ('terminale-cd-chemistry-alcohols', '["alcohol-definition-nomenclature","alcohol-classes","alcohol-preparation","sodium-dehydration-combustion","mild-oxidation","redox-equations-polyols","unknown-alcohol-mission"]');
...
with raw_rewards as (
  ...
  from alcohols_manifest as manifest
...
with reward_weights as (
  ...
  join alcohols_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
