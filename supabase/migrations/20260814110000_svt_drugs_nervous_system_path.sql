-- Publie la leçon de SVT « Les drogues et le système nerveux »
-- (Terminale C, thème La communication dans l'organisme).
--
...
-- (apps/web/src/data/terminalCSvtDrugsPath.ts) et du registre XP de l'API
-- (apps/api/src/curriculum.ts), afin que l'XP affiché corresponde à l'XP attribué.
-- Répartition normalisée attendue :
--   770 / 940 / 1110 / 1200 / 1280 / 1370 / 1540 / 1790 = 10 000.

create temporary table svt_drugs_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_drugs_manifest (path_id, lesson_ids) values
  ('terminale-c-svt-l2-drugs-nervous-system', '["experimental-effects","normal-synapse","stimulant-mechanisms","inhibitory-modulation","health-social-prevention","amphetamine-cocaine-assessment","consolidation-vocabulary","cocaine-dopamine-mission"]');
...
with raw_rewards as (
  select
    manifest.path_id,
    expanded.lesson_id,
    (array[45, 55, 65, 70, 75, 80, 90, 105])[expanded.position]::integer as xp_awarded
  from svt_drugs_manifest as manifest
...
with reward_weights as (
  ...
  join svt_drugs_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
