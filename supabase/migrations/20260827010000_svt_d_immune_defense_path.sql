...
-- (Terminale D, chapitre 11 du catalogue ; le support source porte « L6 »).
--
...
-- (apps/web/src/data/terminalDSvtImmuneDefensePath.ts) et du registre XP
-- de l’API (apps/api/src/curriculum.ts). Répartition normalisée attendue :
--   550 / 670 / 790 / 850 / 910 / 980 / 1100 / 1220 / 1340 / 1590 = 10 000.

create temporary table svt_d_immune_defense_manifest (
  path_id text primary key,
  lesson_ids jsonb not null
) on commit drop;

insert into svt_d_immune_defense_manifest (path_id, lesson_ids) values
  ('terminale-d-svt-l11-immune-defense', '["barriers-innate-defense","inflammation-recruitment","phagocytosis-complement","adaptive-specificity-transfer-experiments","antibodies-humoral-response","lymphocytes-lymphoid-organs","grafts-self-nonself-hla","antigen-presentation-clonal-expansion","adaptive-effectors-memory-cooperation","immune-defense-final-mission"]');
...
with raw_rewards as (
  ...
  from svt_d_immune_defense_manifest as manifest
...
with reward_weights as (
  ...
  join svt_d_immune_defense_manifest as manifest using (path_id)
),
base_allocations as (
  select
    path_id,
...
where progress.path_id = reward.path_id
  ...
  and progress.path_id = 'terminale-d-svt-l11-immune-defense';
